...
{
	u8 res;
	OLED_ShowString(1, 1, "HHHHH", 16, 1);
	OLED_Refresh();
...
{
	u8 i=0;
	usart_sendstr(USART2,"AT+CWMODE=1",sizeof("AT+CWMODE=1"));
	...
	usart_sendstr(USART2,"AT+CWJAP_CUR=\"DESKTOP-CJGM9VL 6508\",\"12345678\"",sizeof("AT+CWJAP_CUR=\"DESKTOP-CJGM9VL 6508\",\"12345678\""));
	for(i=0;i<10&&can==0;i++){drv_systick_delay_ms(100);}
	if(can==1){OLED_ShowString(0,0, USART_RX_BUF, 16, 1);OLED_Refresh();can=0;}
	else {OLED_ShowString(0,0,"error:AT+CWMODE=1", 16, 1);OLED_Refresh();return;}
	
	usart_sendstr(USART2,"AT+CIPSTART=\"TCP\",\"192.168.137.227\",8080",sizeof("AT+CIPSTART=\"TCP\",\"192.168.137.227\",8080"));
	for(i=0;i<10&&can==0;i++){drv_systick_delay_ms(100);}
