...
{
	u8 res;
	if(can==1&&USART_GetITStatus(USART2,USART_IT_RXNE)){
		//memset((char*)GET_BUF,0,sizeof(GET_BUF));//接收到消息时清空上一次信息
		...
		if((USART2_RX_STA&0x8000)==0)//接收未完成，&0x8000是为了判断最高位是否为1
		{
			if(USART2_RX_STA&0x4000)//判断是否接收到了0x0d,接收到了0x0d那么最高位的下一位会变为1
			{
				...
				if(res!=0x0a)USART2_RX_STA=0;//接收错误，重新开始
					...
					get=1;
					i=0;
					memset((char*)USART2_RX_BUF,0,sizeof(USART2_RX_BUF));
					USART2_RX_STA=0;
					TIM_Cmd(TIM5, ENABLE); 
				}
			}
			else
			{
				...
				else
				{
					...
					USART2_RX_STA++;
				 	...
				 	if(USART2_RX_STA>7){GET_BUF[i]=res;i++;}
				}
			}
		}	
	}
		...
		if((USART2_RX_STA&0x8000)==0)//接收未完成，&0x8000是为了判断最高位是否为1
		{
			if(USART2_RX_STA&0x4000)//判断是否接收到了0x0d,接收到了0x0d那么最高位的下一位会变为1
			{
				...
				if(res!=0x0a)USART2_RX_STA=0;//接收错误，重新开始
					...
					USART2_RX_STA|= 0x8000;//接收完成，给最高位置1
					//if(strcmp((const char*)USART2_RX_BUF,"connect")==0)can=1;
					memset((char*)USART2_RX_BUF,0,sizeof(USART2_RX_BUF));
					USART2_RX_STA=0;
				}
			}
