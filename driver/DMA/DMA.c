...
void My_DMA_Init(DMA_Stream_TypeDef * DMA2_Streamx,u32 chx,u32 *AddrA,u32 *AddrB,u16 Size)
{
	...
	DMA_InitStruct.DMA_PeripheralBaseAddr=(uint32_t)AddrA;//外设站点的基地址,0x20000000(SRAM)
	DMA_InitStruct.DMA_PeripheralDataSize=DMA_PeripheralDataSize_Word;//以字节的方式传输
	DMA_InitStruct.DMA_PeripheralInc=DMA_PeripheralInc_Enable;//地址自增
	DMA_InitStruct.DMA_Memory0BaseAddr=(uint32_t)AddrB;//存储器基地址参数
	DMA_InitStruct.DMA_MemoryDataSize=DMA_MemoryDataSize_Word;
	DMA_InitStruct.DMA_MemoryInc=DMA_MemoryInc_Enable;
	DMA_InitStruct.DMA_DIR=DMA_DIR_MemoryToMemory;//寄存器到外设
	...
	DMA_Init(DMA2_Streamx, &DMA_InitStruct); 
	DMA_Cmd(DMA2_Streamx, ENABLE);//初始化后立即工作
...
extern u32 buf[];
extern u32 dma_buf[];
void dmadata_cmpare(void)
{
	u8 i;
	for(i=0;i<10;i++){
		if(buf[i]!=dma_buf[i]){
			printf("dmabuf[%d]_err\r\n",i);
			return;
		}
		printf("dmabuf[%d] = 0x%x\r\n",i,dma_buf[i]);
	}
	printf("dmabuf_OK\r\n");
}
